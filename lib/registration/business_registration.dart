...
class BusinessRegistration extends StatefulWidget {
  ...
  const BusinessRegistration({super.key, this.hasAcceptedTerms = false});
...
class _BusinessRegistrationState extends State<BusinessRegistration> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      String? birUrl;
      if (_birImage != null) {
        birUrl = await _uploadImage(_birImage!, userId);
      }

      // Save to business_registration
      await FirebaseFirestore.instance.collection('business_registration').doc(userId).set({
        'full_name': _fullNameController.text.trim(),
        'business_name': _businessNameController.text.trim(),
        'email': _emailController.text.trim(),
        'phone': _phoneController.text.trim(),
        'bir_certificate': birUrl ?? '',
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (context) => const BusinessDonePage()),
      );
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
                    ...
                    child: GestureDetector(
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => const BusinessTac(),
                          ),
                        );
                      },
                      child: Text(
  ...
  Widget uploadField(String label) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            Icon(_birImage != null ? Icons.check_circle : Icons.cloud_upload_outlined,
                color: _birImage != null ? Colors.green : Colors.grey),
            Text(
              _birImage != null ? 'BIR Selected' : 'Upload BIR Certificate',
              style: GoogleFonts.poppins(
                  color: _birImage != null ? Colors.green : Colors.grey),
            ),
          ],
